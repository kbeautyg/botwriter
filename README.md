...
1. `/start` — приветствие.
2. `/new` — новый бриф. Бот сначала спрашивает **длину** (~200 / ~400 / ~600 / Авто).
...
6. Inline-кнопки оценки **-10..+10** + «💬 Комментарий» + «💾 В образцы» + «🆕 Новый пост».
7. **💬 Комментарий** — пишешь свободным текстом что не так / что хочешь иначе. LLM извлекает 1-3 директивы (`do`/`don't`), они сохраняются в `UserDirective` и подкладываются в Planner+Writer на всех будущих генерациях.
8. Если оценка ≥ `AUTO_SAVE_AS_EXAMPLE_THRESHOLD` (по умолчанию 7) — пост автоматически попадает в style_examples и Stylist извлекает из него хорошие фразы.
9. Если перед оценкой прислать текст — это считается твоей правкой и сохраняется в `edited_text`.
10. `/directives` — посмотреть все активные правила, которые бот выучил из твоих комментариев.
